...
async function fetchPrices() {
    ...
    for (const def of ASSET_DEFS) {
          ...
          if (raw == null) { prices[def.key] = { year:currentYear, value:null, live:false }; continue; }
...
const UNIQUE_TICKERS = [...new Set(ASSET_DEFS.map(a=>a.ticker))];
...
async function fetchPrices() {
  ...
  let usdInr = 84.0;
  try {
    const fx = await yahooFinance.quote("INR=X");
    usdInr = fx?.regularMarketPrice ?? usdInr;
  } catch(e) { console.warn("FX fallback"); }

  const rawPrices = {};
  await Promise.all(UNIQUE_TICKERS.map(async ticker => {
    try {
      const q = await yahooFinance.quote(ticker, { fields:["regularMarketPrice"] });
      rawPrices[ticker] = q?.regularMarketPrice ?? null;
    } catch(e) { rawPrices[ticker] = null; }
  }));
